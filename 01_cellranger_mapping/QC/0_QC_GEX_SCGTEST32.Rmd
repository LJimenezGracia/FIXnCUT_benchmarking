...
seurat_obj$condition <- as.factor(seurat_obj_metadata$condition)
...
ggdens_libsize <- seurat_obj@meta.data %>%
  ggplot(aes(x = nCount_RNA, fill = tissue, color = tissue)) +
  ...
  facet_grid(sample_protocol ~ ., scales = "free")
...
ggdens_ngenes <- seurat_obj@meta.data %>%
  ggplot(aes(x = nFeature_RNA, fill = tissue, color = tissue)) +
    geom_density(alpha = 0.2) + 
    scale_x_log10() +
...
ggdens_pctmt <- seurat_obj@meta.data %>%
  ggplot(aes(x = pct_mt, fill = tissue, color = tissue)) +
    geom_density(alpha = 0.2) + 
...
ggdens_pctrb <- seurat_obj@meta.data %>%
  ggplot(aes(x = pct_rb, fill = tissue, color = tissue)) +
    geom_density(alpha = 0.2) + 
...
```{r}
# Save Merged Seurat object
#saveRDS(seurat_obj, str_c(path_r_objects, "/scgtest32_filtered.rds"))
#seurat_obj <- readRDS(str_c(path_r_objects, "/scgtest32_filtered.rds"))
```

...
for (protocol in metadata$sample_protocol) {
  ...
  path_general <- here::here("01_cellranger_mapping/subprojects/THREETR_27/jobs")
...
write_csv(x = seqreads_genes_df,
          file = "tables/HumanColon_LibGenesUMIvsReads.csv")
```
...
seqreads_genes_df <- read.csv("tables/HumanColon_LibGenesUMIvsReads.csv")
...
seqreads_genes_df$sample_protocol <- factor(x = seqreads_genes_df$sample_protocol, 
...
gg_genes <- seqreads_genes_df %>%
  ...
  scale_x_continuous(labels = label_number(scale = 1e-6), breaks = seq(0, 80000000, by = 10000000)) +
  theme_classic() +
...
ggsave(plot=gg,  filename = "figs/HumanColon_LibGenesUMIvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```


## Statistics SHORT TERM STORGE
```{r}
seqreads_genes_df <- read.csv("tables/HumanColon_LibGenesUMIvsReads.csv")
...
seqreads_genes_df <- subset(seqreads_genes_df, sample_protocol != "Cryo")
seqreads_genes_df <- subset(seqreads_genes_df, sample_protocol != "Fixed+Cryo")
seqreads_genes_df$sample_protocol <- factor(x = seqreads_genes_df$sample_protocol, 
...
# Drop rows with "Fixed" in the "sample_protocol" column
seqreads_genes_df <- subset(seqreads_genes_df, sample_protocol != "Fresh")
...
cell_info_df$library <- factor(x = cell_info_df$library, levels = c("Fresh", "Fixed", "Cryo", "Fixed+Cryo"))
...
detectedgenes$library_name <- factor(x = detectedgenes$library_name, levels = c("Fresh", "Fixed", "Cryo","Fixed+Cryo"))

...
metadata$library_name[metadata$library_name == "Lung_fixed_cryo"] <- "Fixed+Cryo"
...
seq_list <- c(10000000, 20000000, 30000000, 40000000, 50000000, 60000000, 70000000,
...
write_csv(x = seqreads_genes_df,
          file = "tables/MouseLung_cryo_LibGenesUMIvsReads.csv")
```
...
seqreads_genes_df <- read.csv("tables/MouseLung_cryo_LibGenesUMIvsReads.csv")

seqreads_genes_df$sample_protocol[seqreads_genes_df$sample_protocol == "Cryopreservation"] <- "Cryo"
seqreads_genes_df$sample_protocol[seqreads_genes_df$sample_protocol == "Fix&Cut+Cryopreservation"] <- "Fixed+Cryo"

seqreads_genes_df$sample_protocol <- factor(x = seqreads_genes_df$sample_protocol, levels = c("Fixed", "Cryo", "Fixed+Cryo"))
...
gg_genes <- seqreads_genes_df %>%
  ...
  scale_x_continuous(labels = label_number(scale = 1e-6), breaks = seq(0, 100000000, by = 10000000)) +
  theme_classic() +
...
ggsave(plot=gg,  filename = "figs/MouseLung_cryo_LibGenesUMIvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
seqreads_genes_df$sample_protocol[seqreads_genes_df$sample_protocol == "Cryopreservation"] <- "Cryo"
seqreads_genes_df$sample_protocol[seqreads_genes_df$sample_protocol == "Fix&Cut+Cryopreservation"] <- "Fixed+Cryo"
# Drop rows with "Fixed" in the "sample_protocol" column
seqreads_genes_df <- subset(seqreads_genes_df, sample_protocol != "Fixed")
seqreads_genes_df$sample_protocol <- factor(x = seqreads_genes_df$sample_protocol, levels = c("Cryo", "Fixed+Cryo"))
...
model_cryo_genes <- lm(detected_genes ~ sequenced_reads, data = subset(seqreads_genes_df, sample_protocol == "Cryo"))
model_fixed_genes <- lm(detected_genes ~ sequenced_reads, data = subset(seqreads_genes_df, sample_protocol == "Fixed+Cryo"))

# Obtain confidence intervals for the coefficient of X
ci_model_cryo_genes<- confint(model_cryo_genes)["sequenced_reads", ]
ci_model_fixed_genes <- confint(model_fixed_genes)["sequenced_reads", ]
print("detected_genes")
print(ci_model_cryo_genes)
print(ci_model_fixed_genes)

# Check if confidence intervals overlap
ci_overlap_genes <- ci_model_cryo_genes[2] > ci_model_fixed_genes[1] && ci_model_fixed_genes[2] > ci_model_cryo_genes[1]
ci_overlap_genes


# Fit linear regression models
model_cryo_umis <- lm(detected_umis ~ sequenced_reads, data = subset(seqreads_genes_df, sample_protocol == "Cryo"))
model_fixed_umis <- lm(detected_umis ~ sequenced_reads, data = subset(seqreads_genes_df, sample_protocol == "Fixed+Cryo"))

# Obtain confidence intervals for the coefficient of X
ci_model_cryo_umis <- confint(model_cryo_umis)["sequenced_reads", ]
ci_model_fixed_umis <- confint(model_fixed_umis)["sequenced_reads", ]
print("detected_umis")
print(ci_model_cryo_umis)
print(ci_model_fixed_umis)

# Check if confidence intervals overlap
ci_overlap_umis <- ci_model_cryo_umis[2] > ci_model_fixed_umis[1] && ci_model_fixed_umis[2] > ci_model_cryo_umis[1]
ci_overlap_umis
```
...
write_csv(x = cell_info_df,
          file = "tables/MouseLung_cryo_LibGenes-UMIsvsReadsCell.csv")
```


## # Genes / # Seq reads (per cell)
```{r fig.width=8, fig.height=5}
cell_info_df <- read.csv("tables/MouseLung_cryo_LibGenes-UMIsvsReadsCell.csv")

cell_info_df$library[cell_info_df$library == "Cryopreserved"] <- "Cryo"
cell_info_df$library[cell_info_df$library == "Fixed+Cryopreserved"] <- "Fixed+Cryo"

cell_info_df$library <- factor(x = cell_info_df$library, levels = c("Fixed", "Cryo", "Fixed+Cryo"))
...
ggsave("figs/MouseLung_cryo_LibGenesvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
ggsave("figs/MouseLung_cryo_LibUMIsvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
```{r}
# Load Seurat object
seurat_obj <- readRDS(here::here("02_QC/results/R_objects/FIXnCUT_MouseLung_cryopreserved_merged.rds"))
seurat_obj
```
...
detectedgenes <- detectedgenes_df %>% 
  gather(key = "library_name", value = "detected_genes", `Cryopreserved`:`Fixed+Cryopreserved`)

head(detectedgenes)
write_csv(x = detectedgenes,
          file = "tables/MouseLung_cryo_CumulativeGenes.csv")
```

## Cumulative # Genes / # cells
```{r fig.width=8, fig.height=5}
detectedgenes <- read.csv("tables/MouseLung_cryo_CumulativeGenes.csv")


detectedgenes$library_name[detectedgenes$library_name == "Cryopreserved"] <- "Cryo"
detectedgenes$library_name[detectedgenes$library_name == "Fixed+Cryopreserved"] <- "Fixed+Cryo"

detectedgenes$library_name <- factor(x = detectedgenes$library_name, levels = c("Fixed", "Cryo", "Fixed+Cryo"))
...
ggsave("figs/MouseLung_cryo_CumulativeGenes.svg",
       width = 8, height = 5,
       dpi = 300)
```

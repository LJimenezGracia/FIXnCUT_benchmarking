...
```{r}
seq_list <- c(5000000, 10000000, 15000000, 20000000, 25000000, 30000000, 35000000, 40000000)

seqreads_genes_df <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(seqreads_genes_df) <- c("sequenced_reads", "detected_genes", "sample_protocol")

for (protocol in metadata$sample_protocol) {
  print(protocol)
  gem_id <- unique(metadata$gem_id[metadata$sample_protocol == protocol])

  path_general <- here::here("01_cellranger_mapping/subprojects/SCGTEST_32/jobs")
  ...
  for (proportion in prop_list) {
    ...
    seqreads_genes_df[nrow(seqreads_genes_df)+1, ] <- c(sequenced_reads, detected_genes, protocol)
  }
}
...
write_csv(x = seqreads_genes_df,
          file = "tables/MouseColon_LibGenesvsReads.csv")
```

## # Genes / Seq reads
```{r fig.width=8, fig.height=5}
seqreads_genes_df <- read.csv("tables/MouseColon_LibGenesvsReads.csv")

seqreads_genes_df %>%
  ggplot(aes(x = sequenced_reads, y = detected_genes, color = sample_protocol, fill=sample_protocol)) + 
  geom_point() +
  geom_smooth(se = TRUE) +
...
seqreads_genes_df %>%
  ggplot(aes(x = sequenced_reads, y = detected_genes, color = sample_protocol,  fill=sample_protocol)) + # label = slope_gene,
  geom_point(size=4) +
...
ggsave("figs/MouseColon_LibGenesvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
for (protocol in metadata$sample_protocol) {
  fit1 <- lm(formula =  log10(gene) ~ log10(reads), data = cell_info_df[cell_info_df$protocol == protocol, ])
  #summary(fit1)
  print(protocol)
...
for (protocol in metadata$sample_protocol) {
  fit1 <- lm(formula =  log10(umi) ~ log10(reads), data = cell_info_df[cell_info_df$protocol == protocol, ])
  #summary(fit1)
  slope <- round(coef(fit1)[2], digits = 4)
  print(slope)
...
write_csv(x = cell_info_df,
          file = "tables/MouseColon_LibGenes-UMIsvsReadsCell.csv")
```


## # Genes / # Seq reads (per cell)
```{r fig.width=8, fig.height=5}
cell_info_df <- read.csv("tables/MouseColon_LibGenes-UMIsvsReadsCell.csv")

gg <- cell_info_df %>%
  ggplot(aes(x = reads, y = gene, color = protocol, fill = protocol)) +
...
ggsave("figs/MouseColon_LibGenesvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
gg <- cell_info_df %>%
  ggplot(aes(x = reads, y = umi, color = protocol, fill = protocol)) +
  geom_point(alpha=0.1) + 
  #geom_rug() +
  scale_color_manual(values = palette_color_factor) +
  scale_fill_manual(values = palette_color_factor) +
...
ggsave("figs/MouseColon_LibUMIsvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
```{r}
# Load Seurat object
seurat_obj <- readRDS(here::here("02_QC/results/R_objects/FIXnCUT_MouseColon_merged.rds"))
seurat_obj

seurat_obj$sample_protocol[seurat_obj$sample_protocol == "Colon_fresh"] <- "Fresh"
seurat_obj$sample_protocol[seurat_obj$sample_protocol == "Colon_fixed_24h"] <- "Fixed"
```
...
for (size in size_list) {
  #print(size)
  for (protocol in unique(seurat_obj$sample_protocol)) {
  #print(protocol)
...
detectedgenes <- detectedgenes_df %>% 
  gather(key = "sample_protocol", value = "detected_genes", `Fresh`:`Fixed`)

write_csv(x = detectedgenes,
          file = "tables/MouseColon_CumulativeGenes.csv")
```


## Cumulative # Genes / # cells
```{r fig.width=8, fig.height=5}
detectedgenes <- read.csv("tables/MouseColon_CumulativeGenes.csv")
...
ggsave("figs/MouseColon_CumulativeGenes.svg",
       width = 8, height = 5,
       dpi = 300)
```

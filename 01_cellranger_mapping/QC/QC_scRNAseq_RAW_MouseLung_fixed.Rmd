...
```{r}
seq_list <- c(10000000, 15000000, 20000000, 25000000, 30000000, 35000000, 
              40000000, 45000000, 50000000, 55000000, 60000000)

seqreads_genes_df <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(seqreads_genes_df) <- c("sequenced_reads", "detected_genes", "library", "sample_protocol", "replicate")
...
for (library in metadata$library_name) {
  ...
  sample_protocol <- unique(metadata$sample_protocol[metadata$library_name == library])
  ...
  for (proportion in prop_list) {
    ...
    seqreads_genes_df[nrow(seqreads_genes_df)+1, ] <- c(sequenced_reads, detected_genes, library, sample_protocol, replicate)
  }
}
...
for (protocol in metadata$library_name) {
  fit1 <- lm(formula =  detected_genes ~ log10(sequenced_reads), data = seqreads_genes_df[seqreads_genes_df$library == protocol, ])
  #summary(fit1)
  print(protocol)
...
write_csv(x = seqreads_genes_df,
          file = "tables/MouseLung_fixed_LibGenesvsReads.csv")
```


## # Genes / Seq reads
```{r fig.width=8, fig.height=5}
seqreads_genes_df <- read.csv("tables/MouseLung_fixed_LibGenesvsReads.csv")

seqreads_genes_df %>%
  ggplot(aes(x = sequenced_reads, y = detected_genes, color = sample_protocol, fill=sample_protocol, linetype = replicate, shape=replicate)) + 
  geom_point() +
  geom_smooth(se = TRUE) +
...
seqreads_genes_df %>%
  ...
  geom_point(size=4) +
  scale_y_continuous() + 
  scale_x_log10(labels = scales::label_number(scale = 1e-6), breaks = seq(10000000, 130000000, by = 10000001)) +
...
ggsave("figs/MouseLung_fixed_LibGenesvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
colnames(cell_info_df) <- c("cell", "reads", "umi", "gene", "library", "sample_protocol", "replicate")
...
write_csv(x = cell_info_df,
          file = "tables/MouseLung_fixed_LibGenes-UMIsvsReadsCell.csv")
```


## # Genes / # Seq reads (per cell)
```{r fig.width=8, fig.height=5}
cell_info_df <- read.csv("tables/MouseLung_fixed_LibGenes-UMIsvsReadsCell.csv")

gg <- cell_info_df %>%
  ggplot(aes(x = reads, y = gene, color = sample_protocol, fill = sample_protocol, linetype = replicate)) +
...
ggsave("figs/MouseLung_fixed_LibGenesvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
gg <- cell_info_df %>%
  ggplot(aes(x = reads, y = umi, color = sample_protocol, fill = sample_protocol, linetype = replicate)) +
  geom_point(alpha=0.1) + 
  #geom_rug() +
  scale_color_manual(values = palette_color_factor) +
  scale_fill_manual(values = palette_color_factor) +
...
ggsave("figs/MouseLung_fixed_LibUMIsvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
```{r}
# Load Seurat object
seurat_obj <- readRDS(here::here("02_QC/results/R_objects/FIXnCUT_MouseLung_fixed_merged.rds"))
seurat_obj
```
...
detectedgenes <- detectedgenes_df %>% 
  gather(key = "library_name", value = "detected_genes", `Fixed_1`:`Fresh_2`)

detectedgenes$sample_protocol[detectedgenes$library_name == "Fresh_1" | detectedgenes$library_name == "Fresh_2"] <- "Fresh"
detectedgenes$sample_protocol[detectedgenes$library_name == "Fixed_1" | detectedgenes$library_name == "Fixed_2"] <- "Fixed"

detectedgenes$replicate[detectedgenes$library_name == "Fresh_1" | detectedgenes$library_name == "Fixed_1"] <- "Replicate 1"
detectedgenes$replicate[detectedgenes$library_name == "Fresh_2" | detectedgenes$library_name == "Fixed_2"] <- "Replicate 2"

head(detectedgenes)
write_csv(x = detectedgenes,
          file = "tables/MouseLung_fixed_CumulativeGenes.csv")
```


## Cumulative # Genes / # cells
```{r fig.width=8, fig.height=5}
detectedgenes <- read.csv("tables/MouseLung_fixed_CumulativeGenes.csv")

detectedgenes %>% 
  ggplot(aes(x = num_cells, y = detected_genes, color = sample_protocol, shape = replicate)) +
  geom_point(size=3) +
  scale_color_manual(values = palette_color_factor) +
  scale_fill_manual(values = palette_color_factor) +
...
ggsave("figs/MouseLung_fixed_CumulativeGenes.svg",
       width = 8, height = 5,
       dpi = 300)
```

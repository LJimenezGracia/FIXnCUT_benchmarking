...
seq_list <- c(10000000, 20000000, 30000000, 40000000, 50000000, 60000000, 70000000, 80000000,
              90000000, 100000000, 110000000, 120000000, 130000000)
...
write_csv(x = seqreads_genes_df,
          file = "tables/HumanPBMC_LibGenesvsReads.csv")
```

## # Genes / Seq reads
```{r fig.width=8, fig.height=5}
seqreads_genes_df <- read.csv("tables/HumanPBMC_LibGenesvsReads.csv")
...
ggsave("figs/HumanPBMC_LibGenesvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
cell_info_df$protocol[cell_info_df$protocol == "Fix&Cut_24h"] <- "Fixed"

head(cell_info_df)
write_csv(x = cell_info_df,
          file = "tables/HumanPBMC_LibGenes-UMIsvsReadsCell.csv")
```


## # Genes / # Seq reads (per cell)
```{r fig.width=8, fig.height=5}
cell_info_df <- read.csv("tables/HumanPBMC_LibGenes-UMIsvsReadsCell.csv")
...
ggsave("figs/HumanPBMC_LibGenesvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
ggsave("figs/HumanPBMC_LibUMIsvsReadsCell.svg",
       width = 8, height = 5,
       dpi = 300)
```
...
```{r}
# Load Seurat object
seurat_obj <- readRDS(here::here("02_QC/results/R_objects/FIXnCUT_HumanPBMC_merged.rds"))
seurat_obj
```
...
detectedgenes_df <- rename(detectedgenes_df, "Fixed" = `Fix&Cut_24h`)

detectedgenes <- detectedgenes_df %>% 
  gather(key = "sample_protocol", value = "detected_genes", `Fresh`:`Fixed`)

write_csv(x = detectedgenes,
          file = "tables/HumanPBMC_CumulativeGenes.csv")
```

## Cumulative # Genes / # cells
```{r fig.width=8, fig.height=5}
detectedgenes <- read.csv("tables/HumanPBMC_CumulativeGenes.csv")
...
ggsave("figs/HumanPBMC_CumulativeGenes.svg",
       width = 8, height = 5,
       dpi = 300)
```

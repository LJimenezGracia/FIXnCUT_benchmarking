...
write_csv(x = seqreads_genes_df,
          file = "tables/MouseLung_cryo_LibGenesvsReads.csv")
```

## # Genes / Seq reads

```{r fig.width=8, fig.height=5}
seqreads_genes_df <- read.csv("tables/MouseLung_cryo_LibGenesvsReads.csv")
seqreads_genes_df$library[seqreads_genes_df$library == "Cryopreserved"] <- "Cryo"
seqreads_genes_df$library[seqreads_genes_df$library == "Fixed+Cryopreserved"] <- "Fixed+Cryo"

seqreads_genes_df$library <- factor(x = seqreads_genes_df$library, levels = c("Fixed", "Cryo", "Fixed+Cryo"))
...
seqreads_genes_df %>%
  ggplot(aes(x = sequenced_reads, y = detected_genes, color = library,  fill=library)) + # label = slope_gene,
  geom_point(size=4) +
  scale_x_log10(labels = label_number(scale = 1e-6), breaks = seq(10000000, 130000000, by = 20000000)) +
...
ggsave("figs/MouseLung_cryo_LibGenesvsReads.svg",
       width = 8, height = 5,
       dpi = 300)
```

...
seurat_obj <- readRDS(paste0(path_r_objectsANNOT, "/FIXnCUT_MouseColonNonEpithelial.rds"))
...
seurat_obj$lineages <- as.factor(as.character(seurat_obj$lineages))

ggplot_mts_l <- purrr::map(levels(seurat_obj$lineages), function(lineage){
  seurat <- seurat_obj[, seurat_obj$lineages == lineage]
  df <- seurat@meta.data
  df$density <- get_density(df$pct_mt, df$nFeature_RNA, n = 1000)
  gg <- ggplot(df) +
    geom_point(aes(pct_mt, nFeature_RNA, color = density), size = 0.25) +
    facet_wrap(~sample_protocol) +
    viridis::scale_color_viridis() +
    scale_y_sqrt() +
    scale_x_sqrt(breaks = c(1,10, 25, 50, 75, 100), limits = c(0, 100)) +
    geom_vline(xintercept = max_pctmt, linetype = 2, color = 'red') +
...
cowplot::plot_grid(plotlist = ggplot_mts_l, ncol = 3)
```
...
saveRDS(seurat_obj, str_c(path_r_objects, "/FIXnCUT_MouseColonNonEpithelial_filtered.rds"))

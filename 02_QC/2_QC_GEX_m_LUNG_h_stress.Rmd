---
title: "GEX Quality control: mouse_LUNG_healthy_stress"
...
## Load data
The data used in this Rmarkdown document comes from **OMNISCOPE_39** dataset, filtered matrices were processed with `cellranger v6.1.1`, and the doublet predictions were obtained using `scrublet`.
```{r}
# Load all metadata information
metadata <- read.csv(path_project_metadata)

subproject_folder <- "OMNISCOPE_39"
subproject_folder_n <- "OMNISCOPE39"
tissue <- "LUNG"

# Merging metadata_all tables
metadata <- metadata %>% 
  filter(subproject == subproject_folder & tissue == tissue)
...
```{r}
# Metadata
aggr_csv_path <- paste0(path_cellranger_data, "/data/", subproject_folder_n, "_", tissue, "_aggr.csv")
...
## Counts data
filtered_matrix_path <- paste0(path_cellranger_data, "/", subproject_folder_n, "_", tissue, "/outs/count/filtered_feature_bc_matrix")
expression_matrix <- Read10X(data.dir = filtered_matrix_path)
seurat_obj <- CreateSeuratObject(counts = expression_matrix, project = tissue)
...
saveRDS(seurat_obj, str_c(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_merged.rds"))
#seurat_obj <- readRDS(str_c(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_merged.rds"))
```
...
```{r}
# Save Merged Seurat object
saveRDS(seurat_obj, str_c(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_filtered.rds"))
#seurat_obj <- readRDS(str_c(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_filtered.rds"))
```

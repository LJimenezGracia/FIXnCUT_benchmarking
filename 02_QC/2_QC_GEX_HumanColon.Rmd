...
## Load data
The data used in this Rmarkdown document comes from **THREETR_27** dataset, filtered matrices were processed with `cellranger v6.1.1`, and the doublet predictions were obtained using `scrublet`.
```{r}
# Load all metadata information
metadata <- read.csv(path_project_metadata)

# Merging metadata_all THREETR_25
...
metadata_aggr$library_name[metadata_aggr$library_name == "Human colon_fresh"] <- "Fresh"
metadata_aggr$library_name[metadata_aggr$library_name == "Human colon_fixed"] <- "Fixed"
metadata_aggr$library_name[metadata_aggr$library_name == "Human colon_cryopreserved"] <- "Cryopreserved"
metadata_aggr$library_name[metadata_aggr$library_name == "Human colon_fixed_cryopreserved"] <- "Fixed+Cryopreserved"
...
scrublet_files_list <- purrr::map(unique(gem_ids), function(gem_id) {
  ...
  "02_QC/0_doublet_detection/results/", subproject_folder, "__", gem_id, "_doublet_prediction_scrublet.csv"))
}
)
...
# Rename
seurat_obj$condition <- seurat_obj$sample_protocol
```
...
saveRDS(seurat_obj, str_c(path_r_objects, "/FIXnCUT_HumanColon_merged.rds"))
...
seurat_obj$pct_mt_factor <- ifelse(seurat_obj$pct_mt > 85, "high", "low")

VlnPlot(seurat_obj,
        features = c("nCount_RNA", "nFeature_RNA", "pct_mt"),
        split.by = "pct_mt_factor",
        group.by = "sample_protocol")

seurat_obj$pct_mt_factor <- NULL
```
...
```{r}
# Save Merged Seurat object
saveRDS(seurat_obj, str_c(path_r_objects, "/FIXnCUT_HumanColon_filtered.rds"))
#seurat_obj <- readRDS(str_c(path_r_objects, "/FIXnCUT_HumanColon_filtered.rds"))
```

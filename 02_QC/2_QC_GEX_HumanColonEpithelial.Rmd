...
seurat_obj <- readRDS(paste0(path_r_objectsANNOT, "/FIXnCUT_HumanColonEpithelial.rds"))
```
...
ggplot_mts <- purrr::map(levels(seurat_obj$sample_protocol), function(protocol){
  ...
  df <- seurat@meta.data
  df$density <- get_density(df$pct_mt, df$nCount_RNA, n = 1000)
  gg <- ggplot(df) +
    geom_point(aes(pct_mt, nCount_RNA, color = density), size = 0.25) +
...
ggplot_mts <- purrr::map(levels(seurat_obj$sample_protocol), function(protocol){
  seurat <- seurat_obj[, seurat_obj$sample_protocol == protocol]
  df <- seurat@meta.data
  df$density <- get_density(df$pct_mt, df$pct_rb, n = 1000)
  gg <- ggplot(df) +
    geom_point(aes(pct_mt, pct_rb, color = density), size = 0.25) +
    viridis::scale_color_viridis() +
    scale_y_sqrt() +
    scale_x_sqrt(breaks = c(1,10, 25, 50, 75, 100), limits = c(0, 100)) +
    geom_vline(xintercept = max_pctmt, linetype = 2, color = 'red') +
...
## Fraction of ribosomal expression in mitochondria
```{r}
# Calculate percentage mitocondrial genes
seurat_obj$pct_rb_mt <- PercentageFeatureSet(
  seurat_obj,
  pattern = "^MRP[SL]"
)
```
...
```{r fig.height=5, fig.width=14}
ggdens_pctrbmt <- seurat_obj@meta.data %>%
  ggplot(aes(x = pct_rb_mt, fill = sample_protocol, color = sample_protocol)) +
    geom_density(alpha = 0.2) + 
    labs(x = "% Ribosomal Expression from Mitochondria",
         y = "Density") +
    theme_pubr() +
    theme(axis.title = element_text(size = 12),
          axis.text = element_text(size = 10),
...
gghist_pctrbmt <- seurat_obj@meta.data %>%
  ggplot(aes(pct_rb_mt)) +
    geom_histogram(bins = 100) +
    scale_x_continuous(limits = c(0, 100)) +
    labs(x = "% Ribosomal Expression from Mitochondria",
         y = "Frequency") +
...
cowplot::plot_grid(ggdens_pctrbmt, gghist_pctrbmt, ncol = 2)
```

```{r fig.width=25, fig.height=5}
ggplot_mtsrb <- purrr::map(levels(seurat_obj$sample_protocol), function(protocol){
  seurat <- seurat_obj[, seurat_obj$sample_protocol == protocol]
  df <- seurat@meta.data
  df$density <- get_density(df$pct_rb_mt, df$nFeature_RNA, n = 1000)
  gg <- ggplot(df) +
    geom_point(aes(pct_rb_mt, nFeature_RNA, color = density), size = 0.25) +
    viridis::scale_color_viridis() +
    scale_y_sqrt() +
    scale_x_sqrt(breaks = c(1,10, 25, 50, 75, 100), limits = c(0, 100)) +
...
cowplot::plot_grid(plotlist = ggplot_mtsrb, ncol = 4)
```
...
# Test exploring high MT cells

```{r}
seurat_obj_list <- SplitObject(seurat_obj, split.by = "library_name")

seurat_FC_highMT <- subset(seurat_obj_list$`Fixed+Cryopreserved`, subset = pct_mt > 60)
seurat_FC_lowMT <- subset(seurat_obj_list$`Fixed+Cryopreserved`, subset = pct_mt <= 60)
```


```{r}
mt_genes <- c("MT-ND1", "MT-ND2", "MT-CO1", "MT-CO2", "MT-ATP8", "MT-ATP6",
              "MT-CO3", "MT-ND3", "MT-ND4L","MT-ND4", "MT-ND5", "MT-ND6", "MT-CYB") # 13

# Levels of expression of mitochondrial genes counts & data
rowSums(seurat_FC_highMT@assays$RNA@counts[mt_genes, ])
rowSums(seurat_FC_lowMT@assays$RNA@counts[mt_genes, ])

rowSums(seurat_FC_highMT@assays$RNA@data[mt_genes, ])
rowSums(seurat_FC_lowMT@assays$RNA@data[mt_genes, ])
```


```{r}
sum(rowSums(seurat_FC_highMT@assays$RNA@counts))
sum(rowSums(seurat_FC_lowMT@assays$RNA@counts))

sum(rowSums(seurat_FC_highMT@assays$RNA@data))
sum(rowSums(seurat_FC_lowMT@assays$RNA@data))
```


```{r}
# Numbers of detected genes & overlap
summary(Matrix::colSums(seurat_FC_highMT[["RNA"]]@counts > 0))
summary(Matrix::colSums(seurat_FC_lowMT[["RNA"]]@counts > 0))
```
...
saveRDS(seurat_obj, str_c(path_r_objects, "/FIXnCUT_HumanColonEpithelial_filtered.rds"))

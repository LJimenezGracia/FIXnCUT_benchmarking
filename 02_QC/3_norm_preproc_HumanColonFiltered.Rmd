---
title: "Data normalization and pre-processing: HumanColon_filtered"
...
# Parameters
confounder_variables <- c("library_name", "sample_protocol")
confounder_names <- c("Library", "Sample Protocol")
...
# Load Seurat object
seurat_obj_epi <- readRDS(paste0(path_r_objects, "/FIXnCUT_HumanColonEpithelial_filtered.rds"))
seurat_obj_Nepi <- readRDS(paste0(path_r_objects, "/FIXnCUT_HumanColonNonEpithelial_filtered.rds"))
seurat_obj <- merge(seurat_obj_epi, seurat_obj_Nepi)
seurat_obj

seurat_obj$library_name <- factor(x = seurat_obj$library_name, levels = c("Fresh", "Fixed", "Cryopreserved", "Fixed+Cryopreserved"))
seurat_obj$sample_protocol <- factor(x = seurat_obj$sample_protocol, levels = c("Fresh", "Fixed", "Cryopreserved", "Fixed+Cryopreserved"))
```
...
# Cryo HVG
seurat_obj_list$`Cryopreserved` <- FindVariableFeatures(seurat_obj_list$`Cryopreserved`, nfeatures = 3000)
cryo_HVG <- VariableFeatures(seurat_obj_list$`Cryopreserved`)

# Fix&Cut+Cryo HVG
seurat_obj_list$`Fixed+Cryopreserved` <- FindVariableFeatures(seurat_obj_list$`Fixed+Cryopreserved`, nfeatures = 3000)
fixcryo_HVG <- VariableFeatures(seurat_obj_list$`Fixed+Cryopreserved`)
...
UpSetR::upset(
  UpSetR::fromList(list(
    "Fresh" = fresh_HVG,
    "Fixed" = fixed_HVG,
    "Cryopreserved" = cryo_HVG,
    "Fixed+Cryopreserved" = fixcryo_HVG
  )),
  nsets = 6,
  order.by = "freq",
  #group.by = "sets",
  ...
  queries = list(
     list(query = UpSetR::intersects, params = list("Fresh", "Fixed", "Cryopreserved", "Fixed+Cryopreserved"), color = "green", active = T) )
)
```
...
# Non-immune fraction
FeaturePlot(seurat_obj,
            features = c("EPCAM", "KRT8", "KRT18", # Epithelial cells
...
saveRDS(seurat_obj, paste0(path_r_objects, "/FIXnCUT_HumanColon_filtered_lognorm_processed.rds"))
#seurat_obj <- readRDS(paste0(path_r_objects, "/FIXnCUT_HumanColon_filtered_lognorm_processed.rds"))
```
...
```{r}
# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/FIXnCUT_HumanColon_filtered_lognorm_harmony_processed.rds"))
#seurat_obj <- readRDS(paste0(path_r_objects, "/FIXnCUT_HumanColon_filtered_lognorm_harmony_processed.rds"))
```

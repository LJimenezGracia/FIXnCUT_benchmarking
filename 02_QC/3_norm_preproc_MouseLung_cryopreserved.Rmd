---
title: "Data normalization and pre-processing: MouseLung_cryopreserved"
...
color_conditions <- c("#66C2A5", "#8DA0CB", "#E78AC3")
```
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_filtered.rds"))
seurat_obj$library_name <- factor(x = seurat_obj$library_name, levels = c("Fixed", "Cryopreserved", "Fixed+Cryopreserved"))
seurat_obj$sample_protocol <- factor(x = seurat_obj$sample_protocol, levels = c("Fixed", "Cryopreserved", "Fixed+Cryopreserved"))
seurat_obj
```
...
fixedcryo_HVG <- VariableFeatures(seurat_obj_list$`Fixed+Cryopreserved`)
...
UpSetR::upset(
  UpSetR::fromList(list(
    ...
    "Fixed+Cryopreserved" = fixedcryo_HVG
  )),
  nsets = 6,
  order.by = "freq",
  #group.by = "sets",
  ...
  queries = list(
     list(query = UpSetR::intersects, params = list("Fixed", "Cryopreserved", "Fixed+Cryopreserved"), color = "green", active = T) )
)
```
...
saveRDS(seurat_obj, paste0(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_lognorm_processed.rds"))
#seurat_obj <- readRDS(paste0(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_lognorm_processed.rds"))
```

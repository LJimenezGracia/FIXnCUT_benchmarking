---
title: "Data normalization and pre-processing: mouse_LUNG_healthy_stress"
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_filtered_lowQ.rds"))
seurat_obj
```
...
FeaturePlot(seurat_obj,
            features = c("Ptprc", # CD45+, immune cells
                         ...
                         "Cd8b1", "Nkg7", # CD8+ T cells, cytotoxic
                         "Klrb1c", # NK cells
                         "Cd19", "Cd79a", # B cells
...
saveRDS(seurat_obj, paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_lognorm_processed.rds"))
#seurat_obj <- readRDS(paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_lognorm_processed.rds"))
```
...
seurat_obj_integrated <- seurat_obj %>%
  harmony::RunHarmony(group.by.vars = c("library_name"))
...
FeaturePlot(seurat_obj_integrated,
            features = c("Epcam", # Epithelial cells
                        "Pecam1", "Lyve1", # Endothelial cells
                        "Car4", # Pulmonary Endothelial cells 
                        "Vegfc", "Prox1", # Venous / Lymphatic Endothelial cells 
                         "Col1a1", # Stromal cells
                         "Krt5", # Basal cells
                        "Hopx", "Rtkn2", "Ager", "Cav1", "Emp2", # Pneumocytes Type I
                         "Sftpb", "Sftpc", "Sftpd", "Lamp3", "Napsa", "Sftpa1", # Pneumocytes Type II
                         "Tuba1a", # Ciliated cells
                        "Scgb1a1", "Bpifb1", "Muc15", "Scgb3a1",  # Club cells /  Clara cells
                        "Cd24a", "Foxj1", # Ciliated columnar tracheobronchial cells or Ciliated epithelium
                        "Muc5ac",  # Goblet cells, columnar epithelial cells
                        "Pdgfrb", # Axin2+ myofibroblast progenitors (AMPs)
                        "Acta2", "Actg2", "Cnn1", #Smooth muscle,
                        "Wt1" # mesothelium
                         ),
            order = T,
            ncol = 6)
...
```{r}
# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_lognorm_harmony_processed_lowQ.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_lognorm_harmony_processed_lowQ.rds"))
```

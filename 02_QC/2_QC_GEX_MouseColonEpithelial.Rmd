...
seurat_obj <- readRDS(paste0(path_r_objectsANNOT, "/FIXnCUT_MouseColonEpithelial.rds"))
seurat_obj$sample_protocol <- as.factor(seurat_obj$sample_protocol)
```
...
cowplot::plot_grid(plotlist = ggplot_mtsrb, ncol = 2)
```
...
seurat_FC_highMT <- subset(seurat_obj_list$Fixed, subset = pct_mt > 60)
seurat_FC_lowMT <- subset(seurat_obj_list$Fixed, subset = pct_mt <= 60)
```
...
saveRDS(seurat_obj, str_c(path_r_objects, "/FIXnCUT_MouseColonEpithelial_filtered.rds"))
#seurat_obj <- readRDS(str_c(path_r_objects, "/FIXnCUT_MouseColonEpithelial_filtered.rds"))
```

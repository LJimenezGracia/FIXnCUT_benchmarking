...
# Parameters
comparison_name_list <- c("F-C", "FC-Fixed", "FC-C"
                          )
comparison_info_list <- c("MouseLung: F vs C", "MouseLung: F+C vs F", "MouseLung: F+C vs C"
                            )
...
sccoda_effect_df_list <- purrr::map2(comparison_name_list, 
  ...
  file_path <- paste0(path_r_tables, "/MouseLung_cryo_scCODA_", comparison_name, "_effect.tsv")
...
cells_order <- c("T cells", "NK cells", "Monocytes", "Macrophages Apoe+", 
...
# Dotplot immune
```{r}
sccoda_effect_global_df$comparison_label[sccoda_effect_global_df$comparison_label == "F vs C"] <- "Fixed\nvs Cryo"
sccoda_effect_global_df$comparison_label[sccoda_effect_global_df$comparison_label == "F+C vs C"] <- "Fixed+Cryo\nvs Cryo"
sccoda_effect_global_df$comparison_label[sccoda_effect_global_df$comparison_label == "F+C vs F"] <- "Fixed+Cryo\nvs Fixed"
```
...
gg_dotplot_effect_glob <- sccoda_effect_global_df %>%
  filter(comparison_fraction == "MouseLung") %>% 
    ...
    labs(title = "scCODA: MouseLung_cryo",
         color = "Log2FC",
...
fig_path <- paste0(path_r_figs, "/MouseLung_cryo_scCODA_dotplot_celltype.png")

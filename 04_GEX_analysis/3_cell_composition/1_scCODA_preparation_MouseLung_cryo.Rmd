...
celltype_prop_df <- seurat_obj_metadata %>% 
  select(c("library_name", "celltypes")) %>%
  dplyr::count(celltypes, library_name) %>% # only computing number of cells, not percentages
  reshape2::dcast(library_name~celltypes) %>% # change rows to columns
  mutate_if(is.numeric, ~replace(., is.na(.), 0)) # replacing NA by 0
head(celltype_prop_df)

celltype_prop_df_metadata <- seurat_obj_metadata %>% 
   select(c("library_name", "sample_protocol")) %>% 
   unique()
...
openxlsx::write.xlsx(celltype_counts,
                     file = paste0(path_r_tables_out, "/FIXnCUT_MouseLung_cryopreserved_clustering_annotation_cleaned_counts.xlsx"))
```

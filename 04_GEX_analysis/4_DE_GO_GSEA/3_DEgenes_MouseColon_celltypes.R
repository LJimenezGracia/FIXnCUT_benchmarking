...
# between the different groups associated to a particular library_name, and
# to find transcriptional signatures associated to that library_name.
...
## Load data
seurat_obj <- readRDS(paste0(path_r_objects_in, "/FIXnCUT_MouseColon_filtered_post_clustering_annotation.rds"))
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/FIXnCUT_MouseColon_DEgenes_cells_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/FIXnCUT_MouseColon_DEgenes_cells_", comparison_name, ".csv")
  path_file_split <- paste0(path_r_tables, "/FIXnCUT_MouseColon_DEgenes_cells_", comparison_name, "_UPDOWN.xlsx")
  
  # DE analysis considering cell-types separately
  ## Subset data
  cells_to_keep <- (seurat_obj$library_name == group_1 | seurat_obj$library_name == group_2)
  ...
  DE_genes_bycell <- purrr::map(levels(seurat_obj_subset$celltypes), function(celltype) {
    ...
    Idents(seurat_obj_subset) <- "library_name"

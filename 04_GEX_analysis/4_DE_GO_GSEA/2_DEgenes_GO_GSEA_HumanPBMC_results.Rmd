...
for (index in 1:length(comparison_name_list)) {
  ...
  # Load data
  path_object <- paste0(path_r_objects, "/FIXnCUT_HumanPBMC_DEgenes_", comparison_name, ".rds")
  ...
  # Save results
  path_volcanoplot <- paste0(path_r_figs, "/FIXnCUT_HumanPBMC_DEA_", comparison_name, ".svg")
  ggsave(filename = path_volcanoplot, plot = gg_volcanoplot, width = 9, height = 6, dpi = 300)
}
```
...
for (index_1 in 1:length(comparison_name_list)) {
  ...
  # Save results
  path_GSEArds <- paste0(path_r_objects, "/GSEA/FIXnCUT_HumanPBMC_GSEA_", comparison_name, "_", category_db ,".rds")
  saveRDS(fgseaRes_split, file = path_GSEArds)
  
  path_GSEAtable <- paste0(path_r_figs, "/GSEA/FIXnCUT_HumanPBMC_GSEAtop_", comparison_name, "_", category_db , ".svg")
  ggsave(plot = gg_GSEAresults, filename = path_GSEAtable, width = 15, height = 10, dpi = 300)
  
  results_path <- paste0(path_r_tables, "/GSEA/FIXnCUT_HumanPBMC_GSEA_", comparison_name, "_", category_db ,".xlsx")
  openxlsx::write.xlsx(fgseaRes_split, file = results_path)
}
```

## Exploring GSEA on Fresh
```{r}
...
# WP_OXIDATIVE_STRESS
p1 <- plotEnrichment(canonical_pathways[["WP_OXIDATIVE_STRESS"]], ranked_genes) +
   labs(title="WP_OXIDATIVE_STRESS", x = "", y = "Enrichment Score") +
   theme(plot.margin = unit(c(1,1,0,1), "cm"),
         axis.text.x = element_blank(),
         axis.ticks.x = element_blank()
         )
...
p1 <- plotEnrichment(canonical_pathways[["GOBP_CELLULAR_RESPONSE_TO_CHEMICAL_STRESS"]], ranked_genes) +
   labs(title="GOBP_CELLULAR_RESPONSE_TO_CHEMICAL_STRESS", x = "", y = "Enrichment Score") +
   theme(plot.margin = unit(c(1,1,0,1), "cm"),
         axis.text.x = element_blank(),
         axis.ticks.x = element_blank()
         )

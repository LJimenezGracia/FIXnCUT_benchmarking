...
## Load data
seurat_obj <- readRDS(paste0(path_r_objects_in, "/FIXnCUT_HumanPBMC_clustering_annotation_cleaned.rds"))
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/FIXnCUT_HumanPBMC_DEgenes_cells_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/FIXnCUT_HumanPBMC_DEgenes_cells_", comparison_name, ".csv")
  path_file_split <- paste0(path_r_tables, "/FIXnCUT_HumanPBMC_DEgenes_cells_", comparison_name, "_UPDOWN.xlsx")
  
  # DE analysis considering cell-types separately
  ## Subset data
  cells_to_keep <- (seurat_obj$condition == group_1 | seurat_obj$condition == group_2)
  ...
  DE_genes_bycell <- purrr::map(levels(seurat_obj_subset$celltypes), function(celltype) {
    ...
    Idents(seurat_obj_subset) <- "condition"

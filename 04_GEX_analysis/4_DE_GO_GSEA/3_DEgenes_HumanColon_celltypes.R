...
comparison_name_list <- c("Fixed_vs_Fresh", "Fixed_vs_Cryo", "FixedCryo_vs_Cryo", "FixedCryo_vs_Fixed")
group_1_list <- c("Fixed", "Fixed", "Fixed+Cryopreserved", "Fixed+Cryopreserved")
group_2_list <- c("Fresh", "Cryopreserved", "Cryopreserved", "Fixed")

## Load data
seurat_obj <- readRDS(paste0(path_r_objects_in, "/FIXnCUT_HumanColon_filtered_clustering_annotation_cleaned.rds"))
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/FIXnCUT_HumanColon_DEgenes_cells_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/FIXnCUT_HumanColon_DEgenes_cells_", comparison_name, ".csv")
  path_file_split <- paste0(path_r_tables, "/FIXnCUT_HumanColon_DEgenes_cells_", comparison_name, "_UPDOWN.xlsx")

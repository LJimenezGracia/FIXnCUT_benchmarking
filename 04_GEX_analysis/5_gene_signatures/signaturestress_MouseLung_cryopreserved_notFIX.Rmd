...
# Color palette
palette_color_factor <- c("Cryo" ="#8DA0CB",
                          "Fixed+Cryo" = "#E78AC3")
...
```{r}
# Load Seurat object
seurat_obj <- readRDS(str_c(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_clustering_annotation_cleaned.rds"))


# Remove FIXED condition!
seurat_obj <- subset(seurat_obj, subset = library_name != "Fixed")
...
seurat_obj
table(seurat_obj$library_name)
table(seurat_obj$library_name, seurat_obj$celltypes)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_vandenBrinketalUPgenes_Ucell_Vlnbycells.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_vandenBrinketalUPgenes_Ucelldensity.svg"),
       width = 10, height = 5,
       dpi = 300)
```

## Comparing signature with Cryo sDEgenes
```{r}
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_vandenBrinketalUPgenes_DEgenes.svg"),
       width = 12, height = 9,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_OFlanaganetalUPgenes_Ucell_Vlnbycells.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_OFlanaganetalUPgenes_Ucelldensity.svg"),
       width = 10, height = 5,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_OFlanaganetalUPgenes_DEgenes.svg"),
       width = 12, height = 9,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_DenisenkoatalUPgenes_Ucell_Vlnbycells.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_DenisenkoatalUPgenes_Ucelldensity.svg"),
       width = 10, height = 5,
       dpi = 300)
```
...
DE_genes_UP_df <- DE_genes %>% 
    dplyr::filter(is_significant & avg_log2FC > 0) %>% 
    dplyr::select(gene, avg_log2FC, p_val_adj)
DE_genes_UP_list <- DE_genes_UP_df$gene
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_DenisenkoetalUPgenes_DEgenes.svg"),
       width = 12, height = 9,
       dpi = 300)
```
...
```{r fig.height=4, fig.width=4}
venndiagram <- VennDiagram::venn.diagram(
  x = list(DE_genes_UP_list, 
           vandenBrinketal_up_genes_list,
           OFlanaganetal_up_genes_list,
           Denisenkoatal_up_genes_list),
  category.names = c("DE Cryo", " ", " ", " "),
  filename = NULL,
  fill = c("#023047", "#ffb703", "#3DED97", "#E3242B"),
  lty = 'blank',
...
UpSetR::upset(
  UpSetR::fromList(list(
    "DEG in Cryo-only" = DE_genes_DOWN_list,
    "DEG in Fixed+Cryo" = DE_genes_UP_list,
    "Warm collagenase-associated" = vandenBrinketal_up_genes_list,
    "Dissociation-associated" = OFlanaganetal_up_genes_list,
    "Warm-dissociation" = Denisenkoatal_up_genes_list
  )),
  order.by = "freq",
  #group.by = "sets",
  ...
  #empty.intersections = "on",
   queries = list(
     list(query = intersects, params = list("DEG in Cryo-only"), color = "#8DA0CB", active = T),
     list(query = intersects, params = list("DEG in Fixed+Cryo"), color = "#E78AC3", active = T)
     )
)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_MassonietalUPgenes_Ucell_Vlnbycells.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_MassonietalUPgenes_Ucelldensity.svg"),
       width = 10, height = 5,
       dpi = 300)
```


### Comparing Massonietal. signature with Cryo sDEgenes
```{r}
path_r_objects <- here::here("04_GEX_analysis/4_DE_GO_GSEA/results/R_objects")
path_object_DEgenes <- paste0(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_DEgenes_FixedCryo_vs_Cryo.rds")
DE_genes <- readRDS(path_object_DEgenes)
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_MassonietalUPgenes_DEgenes.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_BaechleretalUPgenes_Ucell_Vlnbycells.svg"),
       width = 12, height = 8,
       dpi = 300)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_BaechleretalUPgenes_Ucelldensity.svg"),
       width = 10, height = 5,
       dpi = 300)
```


### Comparing Baechleretal. signature with Cryo sDEgenes
```{r}
path_r_objects <- here::here("04_GEX_analysis/4_DE_GO_GSEA/results/R_objects")
path_object_DEgenes <- paste0(path_r_objects, "/FIXnCUT_MouseLung_cryopreserved_DEgenes_FixedCryo_vs_Cryo.rds")
DE_genes <- readRDS(path_object_DEgenes)
...
ggsave(paste0(path_r_figs_out, "/MouseLung_cryopreserved_BaechleretalUPgenes_DEgenes.svg"),
       width = 12, height = 8,
       dpi = 300)
```

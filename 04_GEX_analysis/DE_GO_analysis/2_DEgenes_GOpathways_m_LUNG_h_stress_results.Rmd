...
for (index in 1:length(comparison_name_list)) {
  ...
  # Load data
  path_object <- paste0(path_r_objects, "/omniscope39_m_LUNG_healthy_stress_DEgenes_", comparison_name, ".rds")
...
top_up <- as.character(subset_sig$gene[subset_sig$avg_log2FC > 0][1:20])
top_down <- subset_sig %>% 
  dplyr::arrange(avg_log2FC)
top_down <- as.character(top_down$gene[1:30])
...
volcano_plot <- DEgenes_df %>%
  ...
  geom_vline(xintercept = -threshold_log2FC, color = "black", linetype = "dashed") +
  geom_text_repel(data = subset_sig, aes(label = gene), color = "black", max.overlaps = 30) +
  labs(title = title,
...
# Save results
path_hallmark <- paste0(path_r_tables, "/omniscope39_m_LUNG_healthy_stress_hallmark_enrichment.xlsx")

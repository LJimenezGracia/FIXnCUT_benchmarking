...
for (index in 1:length(comparison_name_list)) {
  ...
  # Load data
  path_object <- paste0(path_r_objects, "/scgtest33_m_LUNG_healthy_DEgenes_", comparison_name, ".rds")
  ...
  print("Number of sDEgenes UP-regulated (& Log2FC > 0.25)")
  #print(sum(DE_genes$avg_log2FC[DE_genes$is_significant == TRUE] > 0))
  print(sum(DE_genes$avg_log2FC[DE_genes$is_significant == TRUE] > 0.25))
  print("Number of sDEgenes DOWN-regulated (& Log2FC < -0.25)")
  #print(sum(DE_genes$avg_log2FC[DE_genes$is_significant == TRUE] < 0))
  print(sum(DE_genes$avg_log2FC[DE_genes$is_significant == TRUE] < -0.25))
...
```{r fig.width=14, fig.height=10}
  ...
  ## Plot
  gg_enrichR_GO_up <- gg_enrichR_GO(paste0("Intersection UP-regulated"), 
               database,
               enrichR_results_up_filtered, 
               n_top = 20)
  print(gg_enrichR_GO_up)
  
  gg_enrichR_GO_down <- gg_enrichR_GO(paste0("Intersection DOWN-regulated"),  
             database,
...
for (index in 1:length(comparison_name_list)) {
  ...
  names(DE_genes_list) <- c(group_1, group_2)
  myList <- list()
...
# Save results
#path_hallmark <- paste0(path_r_tables, "/scgtest33_m_LUNG_healthy_hallmark_enrichment.xlsx")

...
group_1_list <- c("Fix&Cut_24h")
group_2_list <- c("Fresh")

## Load data
seurat_obj <- readRDS(paste0(path_r_objects_in, "/scgtest34_h_PBMC_healthy_cells1_clustering_annotation.rds"))
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_objects_out, "/scgtest34_h_PBMC_healthy_DEgenes_", comparison_name, ".rds")
  path_file <- paste0(path_r_tables, "/scgtest34_h_PBMC_healthy_DEgenes_", comparison_name, ".csv")
  path_file_split <- paste0(path_r_tables, "/scgtest34_h_PBMC_healthy_DEgenes_", comparison_name, "_UPDOWN.xlsx")

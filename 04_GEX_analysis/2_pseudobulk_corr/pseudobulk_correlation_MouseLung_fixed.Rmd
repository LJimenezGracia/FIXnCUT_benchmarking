...
# Modify metadata
metadata$sample_protocol[metadata$sample_protocol == "Lung_fresh_0"] <- "Fresh_1"
metadata$sample_protocol[metadata$sample_protocol == "Lung_fresh"] <- "Fresh_2"
metadata$sample_protocol[metadata$sample_protocol == "Lung_fixed_24h_0"] <- "Fixed_1"
metadata$sample_protocol[metadata$sample_protocol == "Lung_fixed_24h"] <- "Fixed_2"

metadata$sample_protocol[metadata$sample_protocol == "Fresh_1" | metadata$sample_protocol == "Fresh_2"] <- "Fresh"
metadata$sample_protocol[metadata$sample_protocol == "Fixed_1" | metadata$sample_protocol == "Fixed_2"] <- "Fixed"

metadata$replicate[metadata$sample_protocol == "Fresh_1" | metadata$sample_protocol == "Fixed_1"] <- "Replicate 1"
metadata$replicate[metadata$sample_protocol == "Fresh_2" | metadata$sample_protocol == "Fixed_2"] <- "Replicate 2"
...
m <- match(c("Fresh_1", "Fresh_2", "Fixed_1", "Fixed_2"), sce$library_name)
...
ggsave(paste0(path_r_figs_out, "/MouseLung_fixed_Pseudobulk.png"),
...
g <- gridExtra::grid.arrange(grobs = list(pheatmap1[[4]], pheatmap2[[4]], pheatmap3[[4]],  pheatmap4[[4]], pheatmap5[[4]], pheatmap6[[4]]),
             ncol =3)
ggsave(paste0(path_r_figs_out, "/MouseLung_fixed_Pseudobulk_hallmarks.svg"),
       width = 10, height = 6,
       dpi = 300, plot = g)
```
...
# Aggregate the counts per sample_id
# Subset metadata to only include the cluster and sample IDs to aggregate across
groups <- colData(sce)[, c("celltypes", "sample_protocol")]
...
```{r}
# Print out the table of cells in each cluster-sample group
table(sce$celltypes, sce$sample_protocol)
```
...
for (cluster in levels(seurat_obj$celltypes)) {
  ...
  if (sum(seurat_obj$celltypes == cluster) > 100 &&
      all(as.numeric(table(seurat_obj$sample_protocol[seurat_obj$celltypes == cluster])) > 25) ) {
    pb_sub <- t(pb[[cluster]])
...
cowplot::plot_grid(plotlist = list_gg_cells, ncol = 6)
```
...
ggsave(paste0(path_r_figs_out, "/MouseLung_fixed_Pseudobulk_cells_downs.png"),
       width = 14, height = 16,
       dpi = 300)
```
